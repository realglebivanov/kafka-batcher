defmodule KafkaBatcher.Producers.TestProducer do
  ...
  def start_client(client) do
    process_callback(%{action: :start_client, parameters: {client}}, {:ok, self()})
  end

  @impl true
  def start_producer(client, topic_name, config) do
    process_callback(%{action: :start_producer, parameters: {client, topic_name, config}}, :ok)
  end

  @impl true
  def get_partitions_count(client, topic_name) do
    response = {:ok, @partition_counts[topic_name]}
    process_callback(%{action: :get_partitions_count, parameters: {client, topic_name}}, response)
  end

  @impl true
  def do_produce(client, messages, topic, partition, config) do
    process_callback(%{action: :do_produce, parameters: {client, messages, topic, partition, config}}, :ok)
  end
